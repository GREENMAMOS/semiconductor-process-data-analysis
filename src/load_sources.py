...
def load_project3(path: Path) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    summary_name = "계측 Output 전처리" if "계측 Output 전처리" in wb.sheetnames else "Sheet3"
    raw_name = "계측 데이터" if "계측 데이터" in wb.sheetnames else "Sheet1"
    required_sheets = {summary_name, "좌표", raw_name}
    missing = required_sheets.difference(wb.sheetnames)
    if missing:
        raise ValueError(f"missing Project 3 sheets: {sorted(missing)}")

    condition_rows = []
    sheet = wb[summary_name]
    ...
    raw = wb[raw_name]
    offset = 1 if raw_name == "계측 데이터" else 0
    thickness_row = next(
        (
            row
            for row in raw.iter_rows(min_row=2, values_only=True)
            if len(row) >= 3 and row[0] == 1 and row[1 + offset] == "THK1_1_TOP"
        ),
        None,
    )
