...
반도체 증착 공정 데이터를 활용해 공정 센서 안정성과 공정 변수에 따른 박막 두께 및 균일도 변화를 분석한 프로젝트입니다. 공정 데이터를 기준에 맞게 전처리하고 통계량과 시각화를 통해 조건별 차이를 비교하는 데 초점을 두었습니다.
...
±3σ는 Run 간 변동 폭을 비교하기 위한 참고 범위로 사용했습니다.
...
- 다른 주요 공정 조건을 400 W, 400°C, 30 s로 고정한 13개 조건에서 Si2H6 유량과 평균 박막 두께 사이에 강한 양의 상관관계가 관찰되었습니다 (Pearson r = 0.904).
- 전체 조건 중 가장 작은 표준편차는 `155.3101`이었으며, 해당 조건은 Si2H6 `175 sccm`, power `300 W`, 온도 `300°C`, 시간 `40 s`였습니다.
- 산포가 작다는 사실만으로 최적 조건이라고 단정 짓지 말고, 목표 두께와 규격을 함께 확인해야 합니다.

## 3. Representative wafer map

![Representative wafer thickness map](images/wafer_uniformity_map.png)

대표 조건의 49개 측정 위치를 좌표에 배치했습니다. 중심과 가장자리의 차이를 한눈에 확인하고, 중심부와 edge 영역의 두께 분포 차이를 시각적으로 비교할 수 있습니다.
...
공정 데이터 전처리, 변수별 비교, 통계적 산포 확인, wafer 위치 기반 시각화 과정을 통해 데이터에 근거해 공정 상태를 판단하고 개선 후보를 좁히는 데 초점을 맞춘 프로젝트입니다.
